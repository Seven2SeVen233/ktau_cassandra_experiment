...
\author{Haoyang Chen\\ School of Computer Science and Engineering, Tianjin University of Technology}
\date{}
...
\begin{table}[t]
\centering
\caption{Table 1. Comparison of Kendall τ with existing divergence metrics.}
...
Replica lag / staleness [46] & Per-replica scalar & Data age (how far behind) & None for ordering & O(1) per sample & $Orthogonal: lag ≡ 0 while \hat{\tau} > 0 during drift (Fig. A1)$ \\
...
$\tau(\sigma,$ v) = |{ (a, b) : a $\succ$ $\sigma$ b ∧ b $\succ$ v a }|
...
\section{Kendall τ as a Reconciliation Quality Metric}
...
$\tau(\sigma,$ Li) = |{ (a,b) $\in$ $D^{2}$ : a $\succ$ $\sigma$ b ∧ b $\succ$ Li a }|
...
\subsection{Kendall τ and Causal Disorder}
...
Step-function application semantics. $\tau$ measures ordering quality globally and may not perfectly predict binary operation-level outcomes such as CAS success rate. We find strong correlation with cumulative application outcomes (r ≈ $-0.82)$ but weaker correlation with binary success rates. For this reason $\tau$ should be supplemented by operation-level metrics where application semantics are the primary concern.

Ordering-axis monitoring scope. The pair-counting semantics of §5.1 also delimit $\tau's$ monitoring boundary. $\tau(\sigma,$ Li) sums over pairs that appear in both Li and the reference (pairwise additivity, Appendix B.3): operations a replica is missing contribute no pairs, and operations it alone observed are single-observer pairs that majority aggregation resolves in the observer's own order—so per-replica $\hat{\tau}$ detects and localizes ordering corruption (under Byzantine reorder, lag ≡ 0 and Jaccard ≡ 1.0 while $\hat{\tau}$ rises and ranks the faulty replica first; §7.2) but is structurally blind to pure set divergence (missing and phantom operations; $\hat{\tau}$ ≡ 0; §7.2). Divergence therefore decomposes into a set axis—membership, measured by replica lag, Jaccard similarity, and repair volume—and an ordering axis—pairwise order, measured by $\tau;$ complete monitoring fuses both. This decomposition follows from the metric's definition rather than from any implementation choice, which is why §7 reports the two signals as complementary rather than one superseding the other.
...
\subsection{Validation: τ-Optimal Merging Is Practically Achievable}
...
\caption{Fig. 2. Causal decomposition of total τ distance (Proposition 2) on partial ballots without dissemination. Red: τ_causal (causal-constraint violations). Blue: τ_contested (genuine ordering disagreement). Dark bars: low conflict (35% causal density). Light bars: high conflict (10% causal density). τ_causal is identically zero for CF-RP, vector clocks, and pure CRDT at both conflict levels—the constraint holds by construction—while timestamp-based LWW and CRDT+LWW incur 16.2 (high) to 55.3 (low) causal violations per trial on average, which the metric counts exactly; leader overwrite's timestamp-append fallback incurs 2.7–8.3.}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{images/fig3.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{images/fig4.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{images/fig5.png}
\caption{Fig. 5. As the minority partition grows, leader-based overwrite retains fewer operations (90% → 51%). Full-retention strategies (LWW, CF-RP) stay flat on ordering information retention, with CF-RP consistently 8–10 percentage points above LWW.}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{images/fig6.png}
\caption{Fig. 6. Reconciliation time versus conflict-group size (log scale) on partial ballots. CF-RP is sub-millisecond for typical groups (M ≤ 20), 1.35 ms at M = 30, 4.4 ms at M = 50, and ≈21 ms at M = 100—an extreme case; causal pre-locking adds no measurable overhead over plain Ranked Pairs. LWW and VC detection are cheaper but produce the quality losses measured above.}
\end{figure}
...
\subsubsection{Weighted Kendall τ: Accounting for Operation Importance}
The unweighted $\tau$ treats every operation pair equally, but operations carry heterogeneous importance. The weighted distance $\tau_{w}(\sigma)$ = $\sum_{i}$ $\sum_{(a,b)\in L_{i}}$ $w_{\mathrm{ab}}$ $\cdot$ 1[a $\succ\sigma$ b ∧ b $\succ$ Li a] assigns a nonnegative weight $w_{\mathrm{ab}}$ per pair $(w_{\mathrm{ab}}$ ≡ 1 reduces to $\tau);$ pairwise-weighted Kendall correlations [47] and weighted Kemeny aggregation [48] are studied in the ranking literature. Because weights factor out of per-pair contributions, the weighted distance remains a proper metric for symmetric w, and the weighted majority relation on a pair is decided by $\sum_{i}$ $w_{\mathrm{ab}}$ (1[a $\succ$ Li b] $-$ 1[b $\succ$ Li a]).
...
Each log line maps to an operation under the semanticization of §6.1: the issuing node is hashed to one of N = 8 logical replicas (mapped onto the 4 physical nodes); the line timestamp becomes the operation timestamp (the LWW basis); and the Level/Label field determines the type $(INFO\rightarrowcommutative$ incr; $WARN/ERROR\rightarrowset;$ $FATAL/SEVERE/alert\rightarrowconditional$ cas). A causal DAG is constructed from within-node temporal adjacency plus probabilistic cross-node edges, with the cross-edge probability tuned by binary search to match the target causal density; M = 24 operations are sampled per trial into one connected conflict group, each replica's log being a causally consistent subsequence with clock skew bounded at 500 µs and 5\% observation noise, used directly as the partial ballot (obs ≈ 0.9); the random seed is fixed at 42.
...
\begin{table}[t]
\centering
\caption{Table A2. End-to-end quality on trace scenarios (E2, high conflict, N = 8, M = 24, 30-trial means, partial ballots, no dissemination). Ops ret. = operations retained; Info ret. = 1 − τ̂. CF-RP recovers the exact consensus order (100.0%); LWW, CRDT, and CRDT+LWW follow at 99.0% (p = 0.008 vs. CF-RP); vector clocks defer contested decisions to random tie-breaking and fall significantly behind (68.4%, p < 0.001).}
\begin{tabular}{llllllll}
\hline
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{images/fig8.png}
\caption{Fig. A1. Live τ̂ and Jaccard trajectories through the five phases (one trial per drift strength). During drift, τ̂ rises while maxlag ≡ 0 and Jaccard ≡ 1.0; during partition, Jaccard falls while τ̂ ≈ 0: the two signals respond in disjoint failure modes.}
\end{figure}
...
The implementation and all experiment code are publicly available at $https://github.com/Seven2SeVen233/ktau_{\mathrm{cassandra}}_{\mathrm{experiment}}.$
...
\begin{table}[t]
\centering
\caption{Table A5. Synthetic E2 (N = 10, M = 24, n = 15, partial ballots obs = 0.9, no dissemination): ordering-information retention (1 − τ̂) by strategy—mean ± std, 95% CI, and two-sided Wilcoxon p versus CF-RP.}
\begin{tabular}{llll}
\hline
